...
var ARDUINO_PORT = 'COM5'; 
//var ARDUINO_PORT = '/dev/cu.usbmodem1421';
...
SerialPort.list( function( err, ports ) {
	console.log("SERVER > Available ports: ");
    var found = false;
    
	ports.forEach( function(port) {
		console.log( "SERVER > "+port.comName + " - " + port.manufacturer );
        if ( port.comName == ARDUINO_PORT ) found = true;
    } );
    
    if ( found == false ) {
        console.log("SERVER > Could not find port "+ARDUINO_PORT);
		console.log("SERVER > Searching for a possible Arduino");
		arduinoFound = false;
		ports.forEach( function(port) {
			if (  port.manufacturer.indexOf( "Arduino" ) > -1 || port.manufacturer.indexOf( "arduino" ) > -1 ) {
				ARDUINO_PORT = port.comName;
				console.log("SERVER > Found "+ARDUINO_PORT);
				arduinoFound = true;
			} 
		} );
    ...
    if ( arduinoFound == true ) startArduino();
	else console.log("SERVER > No Arduino found :(");
...
function startArduino() {
    ...
    console.log("SERVER > Connecting to port "+ARDUINO_PORT);
...
socket.on( 'request', function( request ) {
	...
	connection.on( 'message', function( message ) {
		...
		if ( arduinoFound == true ) serial.write( message.utf8Data );
	} );
